 ...
 * 
 *  cpp file with all the functions of the algorythm to compress and decompress
 */
...
void huffman::traverse(Node* r, string str) {
    //traversing to the right +='1' , traversing to the left +='0'
...
int huffman::binToDec(string inStr) {
    //to convert binary code to decimal 
    int res = 0;
    for (auto c : inStr) {
        res = res * 2 + c - '0';
    }
...
void huffman::buildTree(char a_code, string& path) {
    //building the huffman tree with the paths and new nodes 
    Node* curr = root;
...
// Compressing order of functions 
...
// Decompressing order of functions
